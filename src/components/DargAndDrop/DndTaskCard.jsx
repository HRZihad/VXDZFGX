...
export const DndTaskCard = ({ id, index,  label, CaseAllocation,  moveFirstBoxCard, task}) => {
  ...
  return (
    ...
    <Box className=" flex flex-col gap-y-11 px-6 py-4 relative mb-10">
                    ...
                    <Box className=" flex items-center justify-between">
                      <Typography className=" line-clamp-2 " sx={{ color: theme.palette.text.black }}>{label}</Typography>
                      <Box className={` w-5 h-5 rounded-full  absolute -right-3 hidden lg:block `} sx={{ bgcolor: theme.palette.primary.cardLightBg }}>
                        {/* parent root  */}
                        <div onDrag={updateXarrow} onStop={updateXarrow} className=" w-[6px] h-[6px] rounded-full bg-[#83A2DB] m-2" id={task} />
                      </Box>
                    </Box>
                  </Box>
